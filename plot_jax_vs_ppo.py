...
def plot_jax_vs_ppo(instance, upto=None, updates=1):

    # load baseline data
    abspath = os.path.dirname(os.path.abspath(__file__))
    abspath = os.path.join(abspath, "logs", "final_results")
    ppo_data = np.genfromtxt(os.path.join(abspath, f'ppo_instance{instance}_mean_{updates}u.csv'), delimiter=',')
    ...
    if jax_data.shape[0] < ppo_data.shape[0]:
        ...
        jax_data = np.concatenate([jax_data, padding], axis=0)
...
if __name__ == '__main__':
    plot_jax_vs_ppo(1, 10000, 20)
    plot_jax_vs_ppo(3, 10000,20)
    plot_jax_vs_ppo(10, 10000, 20)